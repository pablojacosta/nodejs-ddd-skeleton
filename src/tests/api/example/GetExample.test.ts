...
describe("Get Example", () => {
    ...
    it("Success", async () => {
        const response = await request.get("/examples/1").send();

        expect(response.status).toBe(200);
        expect(response.body.id).toStrictEqual(examples[0].id);
        expect(response.body.value).toStrictEqual(examples[0].value);
    });

    it("Fails because example does not exist", async () => {
        const response = await request.get("/examples/99").send();
